...
private theorem unitMass_pos_of_residual (b : ℕ) (hb : 32 ^ 5 ≤ b)
    (hres : 0 < seedStartupResidual b hb)
    (hmass : (2 * (3 : ℝ) ^ (seedConductor b (seedStartupIndex b hb) - 1)) *
      seedStartupResidual b hb <
      ∑ r ∈ unitResidues (seedConductor b (seedStartupIndex b hb)), blockPersistentMark b hb r) :
    (0 : ℝ) < ∑ r ∈ unitResidues (seedConductor b (seedStartupIndex b hb)),
      blockPersistentMark b hb r := by
  exact lt_trans (mul_pos (mul_pos (by norm_num) (pow_pos (by norm_num) _)) hres) hmass

private theorem dyadicUnitTargetSources_of_mass (b : ℕ) (hb : 32 ^ 5 ≤ b)
    (hzero : (0 : ℝ) < ∑ r ∈ unitResidues (seedConductor b (seedStartupIndex b hb)),
      blockPersistentMark b hb r)
    {v y : ℕ} (hy : ¬ 3 ∣ y) (hodd : Odd y) :
    ∃ W : ℝ, 0 < W ∧ W ≤ 27 / 2 ^ 27 ∧ AllScaleSources (2 ^ v * y) W := by
  classical
  obtain ⟨K, hK0, hKlt⟩ := exists_rat_btwn hzero
  have hK : 0 < K := by exact_mod_cast hK0
  refine ⟨((predecessorScore (27 / 2 ^ 27 : ℚ) K y (seedConductor b
    (seedStartupIndex b hb)) (predecessorStart b v) : ℚ) : ℝ),
...
theorem dyadicUnitTargetSources {v y : ℕ} (hy : ¬ 3 ∣ y) (hodd : Odd y) :
    ∃ W : ℝ, 0 < W ∧ W ≤ 27 / 2 ^ 27 ∧ AllScaleSources (2 ^ v * y) W := by
  exact dyadicUnitTargetSources_of_mass survivalSeedSize (by rfl)
    (unitMass_pos_of_residual survivalSeedSize (by rfl)
      (seedStartup_spec _ (by rfl)).2.2 (survival_unitResidues_mass (by rfl))) hy hodd
